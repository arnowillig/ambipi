...
"h1{font-size:1.3em}button{font-size:1.1em;padding:.7em 1.5em;margin:.3em .3em 0 0;border:0;border-radius:10px;cursor:pointer;transition:background .08s,transform .08s}"
...
".box{position:relative;background:#1b1b1b;border:1px solid #333;border-radius:14px;padding:.8em .6em;margin:.8em 0;text-align:center}"
".stat{position:absolute;top:.55em;right:.8em;font-size:.85em;font-weight:bold;color:#888}"
".tabs{display:flex;gap:.4em;margin:.6em 0 0}.tabs button{flex:1;margin:0;background:#222;color:#aaa;border-radius:10px 10px 0 0}.tabs button.act{background:#1565c0;color:#fff}"
".tab{display:none}.tab.act{display:block}"
".toolbar{text-align:left;margin:.4em 0}.toolbar button{font-size:.85em;padding:.4em .9em;margin:0}"
".row{display:flex;gap:.4em;margin-top:.4em}.row button{flex:1;margin:0;padding:.7em .4em}"
".dpad{display:inline-grid;grid-template-columns:repeat(3,66px);grid-template-areas:'. up .' 'lt ok rt' '. dn .';gap:.3em;margin:.6em auto}.dpad button{margin:0;width:66px;padding:.6em 0}"
".logbox{position:relative;margin-top:1em}.logtools{position:absolute;top:.45em;right:.45em;display:flex;gap:.35em;z-index:1}.logtools button{margin:0;padding:.25em .5em;font-size:.9em;background:#222;color:#ccc;border:1px solid #444;border-radius:6px}"
"#log{background:#000;color:#3f6;font:12px/1.4 monospace;padding:.6em 5.4em .6em .6em;border-radius:10px;height:320px;overflow-y:scroll;white-space:pre-wrap;scrollbar-color:#3f6 #222;scrollbar-width:thin}"
"#log::-webkit-scrollbar{width:10px}#log::-webkit-scrollbar-track{background:#222;border-radius:10px}#log::-webkit-scrollbar-thumb{background:#3f6;border-radius:10px}"
".logbox.min #log{height:0;padding-top:0;padding-bottom:0;overflow:hidden}"
"</style></head><body>"
"<h1>ESP32 Remote <small style='opacity:.55;font-size:.6em'>v" FW_VERSION "</small></h1>"
"<div class=tabs><button id=tabc class=act onclick=\"tab('c')\">Controls</button><button id=tabd onclick=\"tab('d')\">Debug</button></div>"
"<div id=pc class='tab act'>"
"<div class=toolbar><button class=off onclick=\"if(confirm('ESP32 neu starten?'))go2('reboot')\">\xE2\x9F\xB3 Reboot</button></div>"
"<div class=box style='padding-top:1.9em'>"
"<div id=st class=stat>\xE2\x80\xA6</div>"
...
"function toggleLog(){var b=document.getElementById('logbox'),m=b.classList.toggle('min');document.getElementById('mintog').innerHTML=m?'&#x2b;':'&#x2212;';}"
"function tab(t){var c=t=='c';document.getElementById('pc').classList.toggle('act',c);document.getElementById('pd').classList.toggle('act',!c);document.getElementById('tabc').classList.toggle('act',c);document.getElementById('tabd').classList.toggle('act',!c);}"
